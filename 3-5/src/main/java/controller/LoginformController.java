...
public class LoginformController {
	...
	public String loginForm() {
		return "loginform/login"; // 출력될 뷰 이름
	}
	...
	public ModelAndView submit(User user, BindingResult br, HttpSession session) {
		...
		try {
			User loginUser = this.shop.getUser(user); // DB에서 사용자 정보 검색
			if (loginUser != null) { // 로그인 성공한 경우
				mav.setViewName("loginform/loginSuccess");
				// menu_headr.jsp 의 loginUser
				mav.addObject("loginUser", loginUser); 
				session.setAttribute("USER_KEY", loginUser); //세션에 저장
				return mav;
			} else { // 로그인 실패한 경우
