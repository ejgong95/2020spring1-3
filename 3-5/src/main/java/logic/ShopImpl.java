...
//////////////장바구니 관련 메서드 시작
	...
	private Sale createSale(User user, Cart cart) {
		...
		Timestamp currentTime = getCurrentTime(); //구매 연월일시분초 설정
		sale.setUpdateTime(currentTime); //구매 시간을 설정
		List<ItemSet> itemList = cart.getItemList(); //장바구니에서 상품 목록을 추출
		for(int i = 0; i < itemList.size(); i++) {
			ItemSet itemSet = (ItemSet) itemList.get(i); //상품 목록에서 상품 추출
	...
	private SaleDetail createSaleDetail(Sale sale, int saleDetailId, 
		...
		return new SaleDetail(sale, new Integer(saleDetailId), itemSet,
				currentTime); 
	}
