...
public class WriteController {
	...
	@RequestMapping(value = "/write/write.html")
	   public ModelAndView write(@Valid Writing writing, BindingResult br, HttpSession session,
	      ...
	      if(writing.getParent_id()==null) {//원글인 경우
	         ...
	         Integer gId = writeDao.selectMaxGroupId();//DB에서 최대 그룹번호 검색
	         if(gId == null) {
	        	 gId = 0;
	         }
	      ...
	      writing.setRegister_date(registerDate); //작성일 설정
	...
	public ModelAndView list(Integer SEQNO, Integer PAGE_NUM) {
		...
		System.out.println(currentPage-(currentPage%10 == 0 ? 10:(currentPage%10)) +1);
		...
		Condition c = new Condition(); //컨디션을 사용해서 글을 검색
		c.setStartRow(startRow);
		c.setEndRow(endRow);
		List<Writing> writeList = writeDao.getWriting(c); //글 검색
	...
	public ModelAndView delete(Integer id) {
		ModelAndView mav = new ModelAndView("home/template");
		Writing writing = writeDao.getImageWriting(id);//게시글 검색
	...
	//게시글을 삭제하는 매핑
	@RequestMapping(value="/write/delete.Do.html")
	public ModelAndView deleteDo(Writing writing) {
		Writing old = writeDao.getImageWriting(writing.getWriting_id());//게시글 검색
		ModelAndView mav = new ModelAndView("home/template");
		if(writing.getPassword().equals(old.getPassword())) {
			//암호가 일치하는 경우
			writeDao.deleteWriting(writing.getWriting_id());//DB에서 삭제
	...
	public ModelAndView modifyDo(Writing writing, HttpSession session) {
		...
		if (old.getPassword().equals(writing.getPassword())) {
			...
			if(fileName.equals("")) { //이미지 변경이 없는경우
				...
				try {
					...
					writing.setImage_name(fileName); //새이름 설정
				}
			}
	...
	public ModelAndView replyForm(Integer id, Integer parent_id, Integer group_id) {
		System.out.println("replayForm -- ");
		ModelAndView mav = new ModelAndView("home/template");
		Writing original = writeDao.getImageWriting(id); //원글 검색
		...
		original.setParent_id(parent_id);
